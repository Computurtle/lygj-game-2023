...
namespace Febucci.UI.Core
{
    ...
    public abstract class TAnimPlayerBase : MonoBehaviour
    {
        ...
        /// <inheritdoc cref="StartShowingText(bool)"/>
        public IEnumerator ShowTextRoutine(string text)
        {
            ShowText(text);
            yield return ShowRemainingCharacters();
        }

        /// <inheritdoc cref="StartShowingText(bool)"/>
        public IEnumerator ShowTextRoutine(string text, out System.Action cancel)
        {
            ShowText(text);
            cancel = SkipTypewriter;
            return ShowRemainingCharacters();
        }
        ...
        /// <inheritdoc cref="StartDisappearingText"/>
        public IEnumerator DisappearTextRoutine() {
            if (!CanStartAnyCoroutine()) yield break;
            if (disappearanceOrientation == DisappearanceOrientation.Inverted && isInsideRoutine)
            {
                Debug.LogWarning("TextAnimatorPlayer: Can't start disappearance routine in the opposite direction of the typewriter, because you're still showing the text! (the typewriter might get stuck trying to show and override letters that keep disappearing)");
                yield break;
        ...
        /// <inheritdoc cref="StartDisappearingText"/>
        public IEnumerator DisappearTextRoutine(out System.Action cancel) {
            void        None()      { }
            IEnumerator DoNothing() { yield break; }
